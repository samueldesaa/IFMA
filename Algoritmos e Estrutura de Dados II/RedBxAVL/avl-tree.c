...
static void avl_tree_print_util_recursive(AVLTreeNode *node, int level) {
    if (node == NULL) {
        return;
    }

    // Imprime os nós da subárvore direita
    avl_tree_print_util_recursive(node->children[AVL_TREE_NODE_RIGHT], level + 1);

    // Imprime o nó da árvore atual
    ...
    avl_tree_print_util_recursive(node->children[AVL_TREE_NODE_LEFT], level + 1);
}


void avl_tree_print_tree(AVLTree *tree) {
    if (tree == NULL || tree->root_node == NULL) {
        printf("Árvore AVL vazia.\n");
        return;
    }
    avl_tree_print_util_recursive(tree->root_node, 0);
}
